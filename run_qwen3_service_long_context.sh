...
if [ $# -eq 1 ]; then
    ...
    export PORT=8011  # 默认端口
fi

# Set environment variables for long context version
export MODEL_PATH="/iflytek/models/qwen/Qwen3-32B"
export HOST="0.0.0.0"
export TENSOR_PARALLEL_SIZE=2  # Adjust based on your GPU count
export GPU_MEMORY_UTILIZATION=0.8  # Reduced for long context processing
export MAX_MODEL_LEN=131072  # Extended context length with YaRN
export SERVED_MODEL_NAME="Qwen3-32B-Long"  # Clean model name for API calls
...
CONTAINER_NAME="qwen3-32b-long-service-${PORT}"

echo "🚀 Starting Qwen3-32B Long Context service..."
echo "📄 Log file: $LOG_FILE"
echo "📍 Server will be available at: http://$HOST:$PORT"
echo "🏷️  Model name: $SERVED_MODEL_NAME"
echo "📏 Extended context: $MAX_MODEL_LEN tokens (YaRN scaling)"
...
# Run the Docker container with YaRN scaling for long context in background
# Use --gpus '"device=5,6"' to specify GPU IDs 5 and 6
echo "🐳 Starting Docker container with YaRN scaling..."
...
if [ $? -eq 0 ]; then
    ...
    echo "   📏 Context: $MAX_MODEL_LEN tokens (YaRN extended)"
    echo "   🔗 YaRN scaling: factor=4.0, original_max=32768"
    echo "   🧠 Reasoning: enabled (deepseek_r1)"
    echo "   🔧 Container: $CONTAINER_NAME"
    echo "   ⚠️  Note: Long context may impact performance on shorter texts"
    echo ""
